...
public class LinkedCMD implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (commandlabel.equalsIgnoreCase("linked")
				|| commandlabel.equalsIgnoreCase("li")
				|| commandlabel.equalsIgnoreCase("l")) {

			if (args.length >= 2) {

				// Permission check
				if (sender.hasPermission("holographicmenus.linked." + args[0]
						+ "." + args[1])) {
	...
	public static void execute(Player player, String command) {
		...
		for (String commandName : commandList) {
			player.performCommand(VariableUtil.commandVariables(commandName,
					player));
		}
	}
		...
		if (asConsole) {
			...
			for (String commandName : commandList) {
				server.dispatchCommand(server.getConsoleSender(),
						VariableUtil.commandVariables(commandName, player));
			}
			...
			for (String commandName : commandList) {
				player.setOp(true);
				player.performCommand(VariableUtil.commandVariables(
						commandName, player));
				player.setOp(false);
			}
