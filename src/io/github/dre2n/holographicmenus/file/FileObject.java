...
public abstract class FileObject {
	...
	protected Object loadObject(Field field, ConfigurationSection cs,
			String path) throws Exception {
		return loadObject(field, cs, path, 0);
	}

	protected Object saveObject(Object obj, Field field,
			ConfigurationSection cs, String path) throws Exception {
		return saveObject(obj, field, cs, path, 0);
	}

	@SuppressWarnings("rawtypes")
	protected Object loadObject(Field field, ConfigurationSection cs,
			String path, int depth) throws Exception {
		Class clazz = getClassAtDepth(field.getGenericType(), depth);
		if (FileObject.class.isAssignableFrom(clazz)
				&& isConfigurationSection(cs.get(path))) {
			...
			if (FileObject.class.isAssignableFrom(subClazz)
					...
					|| subClazz.isEnum()) {
				return getList(field, cs.getConfigurationSection(path), path,
						depth);
			} else {
				return cs.get(path);
			}
	...
	@SuppressWarnings("rawtypes")
	protected Object saveObject(Object obj, Field field,
			ConfigurationSection cs, String path, int depth) throws Exception {
		Class clazz = getClassAtDepth(field.getGenericType(), depth);
		if (FileObject.class.isAssignableFrom(clazz) && isConfigObject(obj)) {
			return getConfigObject((FileObject) obj, path, cs);
	...
	protected Class getClassAtDepth(Type type, int depth) throws Exception {
		if (depth <= 0) {
			...
			if (className.length() >= 6
					&& className.substring(0, 6).equalsIgnoreCase("class ")) {
	...
	@SuppressWarnings("rawtypes")
	protected FileObject getConfigObject(Class clazz, ConfigurationSection cs)
			throws Exception {
		FileObject obj = (FileObject) clazz.newInstance();
		obj.onLoad(cs);
		return obj;
	}
	...
	protected Map getMap(Field field, ConfigurationSection cs, String path,
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected List getList(Field field, ConfigurationSection cs, String path,
			int depth) throws Exception {
		depth++;
	...
	protected Enum getEnum(Class clazz, String string) throws Exception {
		...
		throw new Exception("String " + string
				+ " not a valid enum constant for " + clazz.getName());
	}
	...
	protected ConfigurationSection getConfigObject(FileObject obj, String path,
			ConfigurationSection cs) throws Exception {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected ConfigurationSection getMap(Map map, Field field,
			ConfigurationSection cs, String path, int depth) throws Exception {
		depth++;
		ConfigurationSection subCS = cs.createSection(path);
	...
	@SuppressWarnings("rawtypes")
	protected ConfigurationSection getList(List list, Field field,
			ConfigurationSection cs, String path, int depth) throws Exception {
		depth++;
		ConfigurationSection subCS = cs.createSection(path);
		String key = path;
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				out = saveObject(out, field, cs, path + "." + key + (i + 1),
						depth);
